    ...
    for link in tqdm(grant_df['link'], desc="Extending new grants"):
        if validators.url(link):
            try:
                ...
                grant_row_df['link'] = link  # For merge!
...
def main():
    print("Loading stub and enriched grant data...")
    ...
    print("Scraping latest DOGE grants...")
    new_stub_grant_df = scrape_grants()
    save_csv(new_stub_grant_df, "doge-grant-stub.csv")  # Save latest stub

    print("Identifying new grants to enrich...")
    new_grants = get_new_grants(new_stub_grant_df, enriched_grant_df)
