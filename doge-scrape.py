...
data_key_dict = { # match on the 'id' field
    # ... your existing dictionary entries here (unchanged) ...
...
def safe_concat(dfs):
    """Concatenate a list of DataFrames, skipping empties or all-NA DataFrames."""
    filtered = [df for df in dfs if not df.empty and not df.isna().all(axis=None)]
    if filtered:
    ...
    for fpds_link in tqdm(contract_df.fpds_link.values):
        if validators.url(fpds_link):
            try:
                ...
                temp_df = pd.DataFrame(contract_row_dict, index=[0])
                fpds_df = safe_concat([fpds_df, temp_df])
            except:
                log_row_error('contract', dt, fpds_link)
        # else, skip if not a valid URL
    return safe_concat([contract_df.reset_index().drop('index', axis=1), fpds_df])
    ...
    for link in tqdm(grant_df.link.values):
        if validators.url(link):
            try:
                ...
                # Only concat if grant_row_df is not empty and not all-NA
        ...
        # else, skip if not a valid URL
    return safe_concat([grant_df.reset_index().drop('index', axis=1), usas_df])
...
def update_doge_data():
    ...
    contract_df = safe_concat([pre_contract_df, new_contract_df])
    ...
    grant_df = safe_concat([pre_grant_df, new_grant_df])
    new_property_df['dt_scrape'] = datetime_scrape
    property_df = safe_concat([pre_property_df, new_property_df])
